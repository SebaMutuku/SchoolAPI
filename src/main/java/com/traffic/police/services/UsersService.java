...
public class UsersService {
        ...
        try {
            if (userEXist != null) {
                if (userEXist.getUserstatus().equalsIgnoreCase("Active")) {
                    ...
                    response.setHttpStatus(HttpStatus.ACCEPTED);
                    response.setRequestStatus(true);
                    response.setMessage("Dear ".concat(userEXist.getFullName().concat(", you have been succesfully activated, your new password is  "
                            .concat(decryptPasword(userEXist.getPassword())))));
                    response.setPayload(userEXist);
    ...
    public GeneralResponse reset(GeneralRequest<ApplicationUsersEntity> request) {
        ...
        response.setHttpStatus(HttpStatus.EXPECTATION_FAILED);
        response.setRequestStatus(false);
        String passW = new StringBuilder().append("PassW").append(new CrimeService().getRadom(4)).toString();
        try {
            if (userEXist != null) {
                System.out.println(userEXist.getUserstatus());
                if (userEXist.getUserstatus().equalsIgnoreCase("Active")) {
                    userEXist.setPassword(encryptPassword(passW));
                    usersRepo.save(userEXist);
                    response.setHttpStatus(HttpStatus.ACCEPTED);
                    response.setRequestStatus(true);
                    response.setMessage("Your new Password is:\n".concat(decryptPasword(userEXist.getPassword())));
                    response.setPayload(userEXist);
                } else {
                    response.setRequestStatus(true);
    ...
    public String decryptPasword(String password) {
        String salt = "zvQTRjptMiCf3GxyQCHpss70E0Y6bTIg";
        byte[] decodedPass = Base64.getDecoder().decode(password.getBytes(StandardCharsets.UTF_8));
        byte[] keyBytes = Base64.getDecoder().decode(salt.getBytes());
        SecretKey key = new SecretKeySpec(keyBytes, "DESede");
        byte[] plainPassword, encryptPassword = null;
        String encryptionScheme = "DESede";
        try {
            Cipher cipher = Cipher.getInstance(encryptionScheme);
            cipher.init(Cipher.DECRYPT_MODE, key);
            encryptPassword = cipher.doFinal(decodedPass);
